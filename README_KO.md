...
- **채팅 UI** — 마크다운 스트리밍, 수식, mermaid 다이어그램, diff가 달린 도구 카드, 이미지 붙여넣기, `@파일` 자동완성, 선택한 텍스트로 스레드·마크, 셸 선택 영역/출력을 "Ask AI"나 컨텍스트 칩으로, git 변경사항 보기(줄 클릭으로 리뷰 코멘트 인용, 원클릭 Claude 리뷰), 이전 세션 검색과 이어하기.
- **실제 터미널** — xterm.js + pty. `claude`, `codex`, `vim` 모두 그대로 실행됩니다.
- **기존 로그인 사용** — `claude -p` 프로세스를 실행하므로 API 키와 추가 요금이 없습니다.

## 설치

Linux x86_64 (Ubuntu 22.04+에서 확인). [Claude Code CLI](https://docs.claude.com/en/docs/claude-code)가 `claude`로 설치되어 있고 로그인된 상태여야 합니다.

**패키지** — [최신 릴리스](https://github.com/cowsjh/x-term/releases/latest)에서 내려받습니다:
...
1. 셸에서 프로젝트로 `cd`한 뒤 `Ctrl+A` — 그 디렉토리에서 Claude 채팅이 열립니다.
...
Tauri 2 (Rust + WebKitGTK) 위에 React UI (dockview pane + xterm.js)를 띄웁니다. 채팅 pane마다 `claude -p --input-format stream-json --output-format stream-json` 프로세스를 하나씩 실행하고 이벤트 스트림을 렌더링합니다. 앱은 모든 세션에 MCP 서버(`x-term --mcp`, unix socket)로 노출되며, 세션은 이를 통해 형제 pane을 열고 완료를 기다립니다.
